  ...
  <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-xs space-y-6">
    ...
    <div class="space-y-4 max-h-96 overflow-y-auto pr-2">
      ...
      <div v-if="comments.length === 0" class="py-12 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200 text-gray-400 text-sm">
        Belum ada komentar atau diskusi pada campaign ini.
      </div>
    </div>
...
const loadComments = async () => {
  try {
    const res = await axios.get(`/api/v1/admin/campaigns/${props.campaignId}`);
    if (res.data?.success && res.data.data?.campaign) {
      comments.value = res.data.data.campaign.comments || [];
    } else {
      comments.value = [];
    }
  } catch (err) {
    console.error('Failed to load campaign comments:', err);
    comments.value = [];
  }
};
...
const handleSend = async () => {
  ...
  try {
    const newComment = await postCampaignComment(props.campaignId, newBody.value.trim());
