<template>
  <div>
    ...
    <DataTable :columns="columns" :data="users" :loading="loading" @search="handleSearch">
      ...
      <template #cell-type="{ row }">
        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
          :class="row.type === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'"
        >
          {{ row.type === 'admin' ? 'Admin' : 'Company' }}
        </span>
      </template>
      ...
      <template #cell-roles="{ row }">
        <span v-for="role in row.roles" :key="role" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 mr-1">{{ role }}</span>
      </template>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div class="relative bg-white rounded-xl p-6 shadow-xl max-w-md w-full mx-4">
        ...
        <div class="flex justify-end gap-3">
          ...
          <button @click="deleteUserAction" class="rounded-lg px-4 py-2 text-white bg-red-600 hover:bg-red-700">Delete</button>
        </div>
      </div>
    </div>

    <UserForm :is-open="isModalOpen" :user="selectedUser" @close="closeModal" @saved="fetchData" />
  </div>
</template>
...
const fetchData = () => {
  fetchUsers({ page: pagination.value.current_page, search: searchQuery.value });
};
