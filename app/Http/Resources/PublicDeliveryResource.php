...
class PublicDeliveryResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'turnover' => (float) $resource->turnover,
            'order_count' => (int) $resource->order_count,
            'ad_spend' => (float) $resource->ad_spend,
            'ad_sales' => (float) $resource->ad_sales,
            'roas' => $resource->ad_spend > 0 ? round((float) $resource->ad_sales / (float) $resource->ad_spend, 2) : null,
            'acos' => $resource->ad_sales > 0 ? round((float) $resource->ad_spend / (float) $resource->ad_sales * 100, 2) : null,
            'ad_contribution' => $resource->turnover > 0 ? round((float) $resource->ad_sales / (float) $resource->turnover * 100, 2) : null,
            'average_order_value' => $resource->order_count > 0 ? round((float) $resource->turnover / $resource->order_count, 2) : null,
            ...
            'brand' => $resource->brand ? ['id' => $resource->brand->id, 'name' => $resource->brand->name] : null,
            'pic' => $resource->pic ? ['name' => $resource->pic->name] : null,
            'creator' => $resource->creator ? ['name' => $resource->creator->name] : null,
