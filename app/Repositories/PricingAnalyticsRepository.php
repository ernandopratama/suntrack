...
class PricingAnalyticsRepository
{
     ...
     */
    public function getPricingOverview(int|string|null $companyId = null): array
    {
        $scopeKey = $companyId ?? 'global';

        return $this->cache->remember(['analytics', 'pricing'], "pricing_overview_{$scopeKey}_".now()->format('Y-m-d-H'), 600, function () use ($companyId) {
            ...
            $raw = DB::table('promotion_variant as pv')
                ...
                ->when($companyId !== null, fn ($query) => $query->where('b.company_id', $companyId))
                ->whereNotNull('pv.discount_price')
                ->select([
                    DB::raw('COUNT(*) as total_mappings'),
                    DB::raw('AVG(v.normal_price) as avg_normal_price'),
                    DB::raw('AVG(pv.discount_price) as avg_promo_price'),
                    DB::raw('AVG(v.bottom_price) as avg_bottom_price'),
                    DB::raw('AVG(pv.discount_price - v.bottom_price) as avg_margin_absolute'),
                    DB::raw('MIN(pv.discount_price - v.bottom_price) as min_margin_absolute'),
                    DB::raw('MAX(pv.discount_price - v.bottom_price) as max_margin_absolute'),
                    DB::raw('SUM(pv.discount_price) as total_estimated_revenue'),
                    DB::raw('SUM(pv.discount_price - v.bottom_price) as total_estimated_profit'),
                    DB::raw('COUNT(CASE WHEN pv.discount_price < v.bottom_price THEN 1 END) as margin_violations'),
                ])
                ->first();
     ...
     */
    public function getMarginViolations(int|string|null $companyId = null, int $perPage = 20): array
    {
        $violations = DB::table('promotion_variant as pv')
            ...
            ->when($companyId !== null, fn ($query) => $query->where('b.company_id', $companyId))
            ->whereNotNull('pv.discount_price')
            ->whereRaw('pv.discount_price < v.bottom_price')
            ->select([
                ...
                'pv.discount_price as promo_price',
                'v.bottom_price',
                DB::raw('(v.bottom_price - pv.discount_price) as margin_gap'),
     ...
     */
    public function simulateDiscount(int|string|null $companyId, float $discountPercent): array
    {
        $raw = DB::table('promotion_variant as pv')
            ->join('variants as v', 'pv.variant_id', '=', 'v.id')
            ->join('products as p', 'v.product_id', '=', 'p.id')
            ...
            ->select([
                ...
                DB::raw('COUNT(CASE WHEN pv.discount_price * ? < v.bottom_price THEN 1 END) as projected_violations'),
            ])
